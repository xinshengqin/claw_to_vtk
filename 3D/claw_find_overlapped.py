...
def set_overlapped_status(sol):
    ...
    # Count how many grids there are in each level
    ...
    for state in sol.states:
        ...
        overlapped_status = np.zeros((1, state.q.shape[1],
                                      state.q.shape[2], state.q.shape[3]))
        ...
        for state_fine in sol.states:
            ...
            if ((state_fine.patch.level-1) == level + 1):
                ...
                zlower_fine = state_fine.patch.dimensions[2].lower
                zupper_fine = state_fine.patch.dimensions[2].upper
                x_idx_lower = \
                ...
                y_idx_upper = \
                    min(int(round((yupper_fine-ylower_fine)/dy)) +
                        y_idx_lower, ny)
                z_idx_lower = \
                    max(int(round((zlower_fine - zlower_coarse) /
                            float(dz))), 0)
                z_idx_upper = \
                    min(int(round((zupper_fine-zlower_fine)/dz)) +
                        z_idx_lower, nz)
                # set these cells to 8
                overlapped_status[0, x_idx_lower:x_idx_upper,
                                  y_idx_lower:y_idx_upper,
                                  z_idx_lower:z_idx_upper].fill(8)
